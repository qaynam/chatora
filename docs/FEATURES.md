...
`:q` で閉じるときは Neovim の確認（`Save changes to "…"?`）が出て、Yes を選ぶと `:w` と同じ流れに
なります。1 行目を書いてカーソルが離れた時点でバッファの名前もそのタイトルになるので、この確認も
タイトルで聞きます。ほかのウィンドウが残っていてバッファが隠れるだけのとき（`'hidden'`）は Neovim は
黙って閉じるので、chatora が同じ言葉で聞きます。
