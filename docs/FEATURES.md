...
chatora は各段に中点を描き、3 つの文字の幅の違いを補って、**どの文字で書いた行も同じ段は同じ
深さに見せます**。貼り付けと手打ちが混ざったページでも、箇条書きがばらばらになりません。
ページのバッファでは `tabstop` を 1 にしています。タブ 1 個が 1 段だからです。
...
`[img1] [img2] [img3]` のように画像しか無い行は、web だと横に流れて折り返します。ターミナルでは、
横に並べるか大きく描くかの二択になります。
...
積むときは各画像を行のインデントに揃えるので、階段状にならず一列になります。
