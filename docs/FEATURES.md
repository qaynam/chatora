...
1 行目がタイトルです。web と同じく、1 行目を書き換えるとページの名前が変わります。確定するのは
カーソルがタイトル行を離れたとき（挿入モードなら抜けたとき）か、`:w` したときで、その場で保存まで
済みます。自動保存はタイトルを変えたページには手を出しません。書きかけのタイトルでページを
何度も改名してしまうからです。

ほかのページからリンクされているページを改名すると、リンクも新しいタイトルに書き換えるか
聞きます。書き換えると、プロジェクト中の `[旧タイトル]`、`#旧タイトル`、`[旧タイトル.icon]` が
新しいタイトルになります。そのままにすると、旧タイトルへのリンクは実体のないページを指す
赤リンクになります。

Cosense には同じタイトルのページが 1 つしかありません。既にあるタイトルにすると、web と同じ
3 つから選びます。
