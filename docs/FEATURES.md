...
サイドバーの `n` か `:Chatora new` で、空のページが開きます。web と同じく、1 行目に書いたものが
タイトルになり、最初に保存したときにページができます。タイトルを書く前に保存しようとすると、
その旨を伝えて保存しません。

同じタイトルのページが既にあるときは、そのページに統合するか聞きます。統合すると、このページの
本文（2 行目以降）がそのページの末尾に足されて、そのページが開きます。やめると保存しないので、
タイトルを変えてから保存し直せます。

`:q` で閉じるときは `Save changes to "…"?`（Neovim の確認と同じ言葉です）と聞きます。Yes は `:w` と
同じ流れで、同名のページがあれば統合するか聞き、やめればバッファに戻ります。No は捨てて閉じ、
Cancel は戻ります。1 行目を書いてカーソルが離れた時点でバッファの名前もそのタイトルになるので、
この確認もタイトルで聞きます。

保存するまでのあいだ、バッファには `無題` という仮の名前が付いています。`:Chatora new タイトル`
のようにタイトルを渡せば、これまでどおりそのページを直接開きます。

### タイトルの変更

1 行目がタイトルです。web と同じく、1 行目を書き換えるとページの名前が変わります。カーソルが
タイトル行を離れたとき（挿入モードなら抜けたとき）か `:w` したときに確認が出て、答えるとその場で
保存まで済みます。答えるまでタイトルはサーバーに送りません。そのあいだも本文の自動保存は続きます
が、タイトルは元のままで送ります。

ほかのページからリンクされているページなら、「N 個のページがリンクしています。リンクも書き換え
ますか？」と聞きます。書き換えると、プロジェクト中の `[旧タイトル]`、`#旧タイトル`、`[旧タイトル.icon]`
が新しいタイトルになります。書き換えないと、新しいタイトルのページがそのまま作られ、旧タイトルへの
リンクは実体のないページを指す赤リンクになります。Esc で閉じると何もしません。タイトルを書き直すか、
`:w` でもう一度聞きます。

Cosense には同じタイトルのページが 1 つしかありません。既にあるタイトルにすると、先に次の 3 つから
選びます。

| 選択 | 起きること |
|---|---|
| 統合する | このページの本文（2 行目以降）をそのページの末尾に足し、このページを削除します。統合先のページが開きます |
| 別の名前で保存する | Cosense が末尾に番号を付けて `タイトル_1` のようにします。web でそのまま別のページへ移ったときと同じです |
| やめる | 何もしません。タイトルを書き直すか、`:w` でもう一度聞きます |

統合先のページをほかのウィンドウで開いていれば、足した行は同期で入ってきます。
...
`[` を押すと `[]` になり、その中でページの候補が出ます。候補はプロジェクトのタイトルと、
まだ無いページ（赤リンク）です。

候補が出るのは**素のリンクの中だけ**です。`[* 見出し]` のような装飾記法や、`[$ x^2]`、
`[taro.icon]`、`[https://…]`、`[/別プロジェクト/ページ]` では出ません。候補を選ぶと括弧の
中身がまるごと置き換わるので、記法の中で出すと書いたものを壊してしまうからです。装飾を
書き始めた時点（`[* ` まで打った時点）で候補は止まるので、見出しを書いている最中にメニューが
ちらつくこともありません。

装飾の中に書いたリンクは素のリンクなので、`[* [ページ]]` の内側では候補が出ます。

`#` のあとでも同じ候補が出ます。タイトルに空白があるページは、空白を `_` に替えて挿入します。
Cosense はタイトルの空白と `_` を同じものとして扱うので、`#Side_Kanban` は `[Side Kanban]` と
同じページに当たります。ただし `#` や `[` `]`、全角の空白を含むタイトルはタグで書き表せないので、
タグの候補には出ません。
...
プライベートプロジェクトのアイコンも表示できます。取得した画像は長辺 2048px に縮めてから端末に
渡します。端末は表示中の画像を展開したまま持ち、上限（Ghostty は 320MB）を超えると古いものから
...
ターミナルが前面に出ます（ターミナルは nvim の環境変数から見つけるので、tmux の中でも前面に
出ます）。それ以外のリンクは、控えておいたブラウザへ行きます。chatora が動いて
...
Cosense 以外のリンクを渡すブラウザは、`install` のときにターミナルで一覧から番号で選びます
（今の既定ブラウザに印が付いていて、Enter でそのままです）。あとから変えるのは
`chatora-url-handler browser` で、同じ一覧が出ます。一覧は、https の URL を受け取って HTML を開く
アプリ、つまりシステム設定がブラウザとして出すものと Safari です。スクリプトから使うときは
`browser 'Google Chrome'` や `install --browser com.google.Chrome` のように、アプリ名か bundle id を
直接渡せます。無いアプリを指定すると、そう言って何も変えません。控えたブラウザがあとで消えて
いても、Safari に渡すので、リンクが開かないことはありません。
