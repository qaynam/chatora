...
- **編集はローカル、同期はマージ**
  - 開いているページはバックグラウンドでサーバーと同期する。
  - 取り込みは上書きではなく行 ID ベースの三方向マージなので、書きかけの内容が消えることはおきない
- **記法のハイライト**
  - 装飾・リンク・コードブロック・テーブル・引用・画像を LSP semantic tokens で描くので
  - マークアップはカーソル行以外で隠してあり、cosense webを限りなく再現する。
- **リンク補完と定義ジャンプ**
  - cosense web同様リンク補完や赤リンク対応
  - `[` や `#` で候補、`gd` でページへジャンプできる
- **テロメア**
  - 行ごとの更新の新しさを左端のバーで出す。誰かが書き換えた行、まだ保存していない行が一目で分かる
- **画像のインライン表示と貼り付け**
  - 対応ターミナルなら本文中に描画。クリップボードの画像は`<leader>cv` でそのままアップロード
- **Cosense のエディタ操作**
  - `<C-t>` 日時挿入、`<C-i>` アイコン挿入、`[` の自動ペア、visualモードで `*` や `[` を押して選択を囲む
- **PAT 認証・複数アカウント**
  - macOS Keychain に保存。`:Chatora account` で切り替え
...
任意: [ImageMagick](https://imagemagick.org/) と画像描画プラグイン（[画像の表示](docs/FEATURES.md#画像の表示)）、クリップボード取り出しツール（[画像の貼り付け](docs/FEATURES.md#画像の貼り付け)）。
