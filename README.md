...
- **リンク補完と定義ジャンプ** — `[` や `#` で候補、`gd` でページへ。実体のないページ（赤リンク）は
  色で分かる
- **テロメア** — 行ごとの更新の新しさを左端のバーで出す。誰かが書き換えた行、まだ保存していない
  行が一目で分かる
...
任意: ImageMagick と画像描画プラグイン（[画像の表示](docs/FEATURES.md#画像の表示)）、クリップボード取り出しツール
（[画像の貼り付け](docs/FEATURES.md#画像の貼り付け)）。
...
{
  'qaynam/chatora',
  version = '*',                        -- 最新のリリースタグを追う。省略すると main の先頭
...
`build` は必須。サーバーはリポジトリに入っていない（`dist/` は追跡していない）ので、ここで
ビルドしないと LSP が起動しない。lazy.nvim は**インストール時と更新時の両方**でこれを走らせるので、
Lua 側とサーバー側がズレることはない。更新は `:Lazy update`（自動更新はしない）。
...
機能ごとの詳しい説明は [docs/FEATURES.md](docs/FEATURES.md)。
...
画像の描画には対応ターミナルと描画プラグインの両方が要る（[画像の表示](docs/FEATURES.md#画像の表示)）。

### コードブロックに色が付かない

必要なのは **treesitter のパーサー**で、その言語の LSP ではない。`code:index.php` は
`index.php` をファイル名として読んで `php` に解決し、その言語のパーサーがあるときだけ色を付ける。
無ければ何も起きない（エラーにもならない）ので、`:TSInstall php` を実行するか、
nvim-treesitter に `auto_install = true` を渡しておく。

パーサーが取ってこられる言語なら、色が付かないときに chatora が一度だけ知らせる。

PHP だけは癖がある。tree-sitter の `php` は `<?php` の**外側を HTML として読む**ので、開きタグの
無いスニペットは色が付かない。chatora はそういうブロックを `php_only`（同じ文法をコードから読む
ほう）で読むので、`:TSInstall php php_only` の両方を入れておくとどちらの書き方でも色が付く。

### 保存が競合で止まる

同じ行をサーバー側でも編集している。`]c` で競合行へ飛び、直してから `:w`。
詳しくは[同期と競合](docs/FEATURES.md#同期と競合)。

## 開発

設計と内部の取り決めは [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
