...
| `hl` | `nvim_set_hl` にそのまま渡る（`:colorscheme` 変更後も再適用） |
...
エントリごと無視する。いずれも `vim.notify` で知らせ、プラグインは落とさない。
...
`log = true` にして `:Chatora log`。取得失敗は HTTP status 付きで記録される。描画には対応
ターミナルと描画プラグインの両方が要る（[画像の表示](#画像の表示)）。
